    ...
    <div className={`overflow-hidden rounded-xl border ${s.border} ${s.bg} backdrop-blur-sm ${s.glow ? `shadow-lg ${s.glow}` : ""}`}>
      ...
      <div className="flex items-center gap-3 border-b border-white/5 px-5 py-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-slate-500">API-based seismic risk analysis</p>
        </div>
      ...
      <div className="p-5">
        ...
        <div className="mb-4 overflow-hidden rounded-lg bg-black/20 p-3">
          <div className="mb-1 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-600">Avg: {status.avgScore.toFixed(3)} | Max: {status.maxScore.toFixed(3)}</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg bg-black/20 p-3">
            ...
            <div className={`text-xl font-black ${status.level !== "NONE" ? s.text : "text-slate-600"}`}>
              {(status.maxScore * 100).toFixed(0)}%
            </div>
            <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
              <div className={`h-full rounded-full transition-all duration-500 ${s.bar}`} style={{ width: `${status.maxScore * 100}%` }} />
            </div>
          </div>
          <div className="rounded-lg bg-black/20 p-3">
            ...
            {status.affectedRobots.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {status.affectedRobots.map((id) => (
                  <span key={id} className={`rounded px-1 py-0.5 text-[9px] font-bold ${s.badge}`}>{id}</span>
                ))}
              </div>
            )}
          </div>
